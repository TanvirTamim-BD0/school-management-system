                                ...
                                <select class="select2 browser-default" id="class_id" name="class_id" required>
                                    ...
                                    @foreach($classData as $singleClassData)
                                        @if(isset($singleClassData) && $singleClassData != null)
                                            <option value="{{ $singleClassData->id }}" {{$classId == $singleClassData->id ? 'selected' : ''}}>{{ $singleClassData->class_name }}
                                            </option>
                                        @endif
                                    @endforeach
                                </select>
                                ...
                                <select class="select2 browser-default" name="section_id" id="section_id"
                                    ...
                                    @foreach($sectionData as $singleSectionData)
                                        @if(isset($singleSectionData) && $singleSectionData != null)
                                            <option value="{{ $singleSectionData->id }}" {{$sectionId == $singleSectionData->id ? 'selected' : ''}}>
            ...
            <div class="card-content custom-card-content custom-card-content-for-datatable">
                ...
                <div class="float-right justify-content-end">
                    ...
                    <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button"
                        href="{{route('attendace-of-student.create')}}">
                            ...
                            <td class=" text-center">
                                ...
                                <ul id='dropdown1' class='dropdown-content custom-dropdown-for-action'>
                                    <li>
                                        ...
                                        <a href="{{ route('syllabus.edit',$item->id) }}"><i
                                                class="material-icons">visibility</i>Attendace History</a>
                                        @endif
                                    </li>
                                    <li>
                                        @if(Auth::user()->can('syllabus-edit'))
                                        <a href="{{ route('syllabus.edit',$item->id) }}"><i
                                                class="material-icons">visibility</i>Student Profile</a>
                                        @endif
                                    </li>
                                </ul>
                            </td>
