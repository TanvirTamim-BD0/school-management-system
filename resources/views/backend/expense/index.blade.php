    ...
    <div class="container">
      <div class="row">
        <div class="col s10 m6 l6">
          <h5 class="breadcrumbs-title mt-0 mb-0"><span>Expense List</span></h5>
  ...
  <div class="col s12">
    <div class="card">
        ...
        <div class="card-content custom-card-content custom-card-content-for-datatable">
          <h2 class="card-title">Expense Record List</h2>
          <div class="float-right justify-content-end">
            @if(Auth::user()->can('expense-create'))
            <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button" href="{{route('expense.create')}}">
             <i class="material-icons dp48">add_circle_outline</i> Add Expense
            </a>
            @endif
          </div>
        </div>
            ...
            <table id="classTable" class="display custom-table custom-data-table custom-table-border dt-responsive nowrap table-responsive">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {{Carbon\Carbon::createFromFormat('Y-m-d', $expense->expense_date)->format('d-m-Y')}}
                  </td>
                  <td>{{$expense->expense_amount}}</td>
                  <td>{{$expense->expense_note}}</td>
                  <td class=" text-center">
                    <!-- Dropdown Trigger -->
                    <a class='dropdown-trigger btn custom-dropdown-btn' href='#' data-target='dropdown1{{$expense->id}}'>
                      <i class="material-icons float-right">more_vert</i>
                    </a>
                    <!-- Dropdown Structure -->
                    <ul id='dropdown1{{$expense->id}}' class='dropdown-content custom-dropdown-for-action'>
                      <li>
                        @if(Auth::user()->can('expense-edit'))
                        <a href="{{ route('expense.edit',$expense->id) }}"><i class="material-icons">edit</i>Edit</a>
                        @endif
                      </li>
                      <li>
                        @if(Auth::user()->can('expense-delete'))
                        <form id="delete-form" action="{{ route('expense.destroy', $expense->id) }}" method="POST"
