    ...
    <div class="container">
      <div class="row">
        <div class="col s10 m6 l6">
          <h5 class="breadcrumbs-title mt-0 mb-0"><span>Income List</span></h5>
          <ol class="breadcrumbs mb-0">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a>
            </li>
            <li class="breadcrumb-item"><a href="#">Income</a>
            </li>
            <li class="breadcrumb-item active">Income List
            </li>
          </ol>
        </div>
      </div>
  ...
  <div class="col s12">
    <div class="card">
        ...
        <div class="card-content custom-card-content custom-card-content-for-datatable">
          <h2 class="card-title">Income Record List</h2>
          <div class="float-right justify-content-end">
            @if(Auth::user()->can('income-create'))
            <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button" href="{{route('income.create')}}">
              <i class="material-icons dp48">add_circle_outline</i> Add Income
            </a>
            @endif
          </div>
        </div>
            ...
            <table id="classTable" class="display custom-table custom-data-table custom-table-border dt-responsive nowrap table-responsive">
              ...
              <thead>
                <tr>
                  ...
                  <th class="custom-border-right">Amount</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {{Carbon\Carbon::createFromFormat('Y-m-d', $income->income_date)->format('d-m-Y')}}
                   </td>
                  <td>{{$income->income_amount}}</td>
                  <td>{{$income->income_note}}</td>
                  <td class=" text-center">
                    <!-- Dropdown Trigger -->
                    <a class='dropdown-trigger btn custom-dropdown-btn' href='#' data-target='dropdown1{{$income->id}}'>
                      <i class="material-icons float-right">more_vert</i>
                    </a>
                    <!-- Dropdown Structure -->
                    <ul id='dropdown1{{$income->id}}' class='dropdown-content custom-dropdown-for-action'>
                      <li>
                        @if(Auth::user()->can('income-edit'))
                        <a href="{{ route('income.edit',$income->id) }}"><i class="material-icons">edit</i>Edit</a>
                        @endif
                      </li>
                      <li>
                        @if(Auth::user()->can('income-delete'))
                        <form id="delete-form" action="{{ route('income.destroy', $income->id) }}" method="POST"
