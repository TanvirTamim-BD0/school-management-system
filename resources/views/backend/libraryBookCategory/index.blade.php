...
@section('content')
@section('title') Library Book Category @endsection
    ...
    <div class="container">
      <div class="row">
        <div class="col s10 m6 l6">
          <h5 class="breadcrumbs-title mt-0 mb-0"><span>Library Book Category List</span></h5>
          <ol class="breadcrumbs mb-0">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a>
            </li>
            <li class="breadcrumb-item"><a href="#">Book Category</a>
            </li>
            <li class="breadcrumb-item active">Library Book Category List
            </li>
          </ol>
        </div>
      </div>
  ...
  <div class="col s12">
    <div class="card">
      ...
      <div class="card-content custom-card-content custom-card-content-for-datatable">
        <h2 class="card-title">Book Category Record List</h2>
        <div class="float-right justify-content-end">
          @if(Auth::user()->can('book-category-of-library-create'))
          <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button"
            href="{{route('book-category-of-library.create')}}">
            <i class="material-icons dp48">add_circle_outline</i> Add Book Category
          </a>
          @endif
        </div>
      </div>
      ...
      <div class="card-content-datatable table-responsive">
        <table id="classTable"
          ...
          <tbody>
            ...
            @foreach($libraryBookCategoryData as $item)
              @if(isset($item) && $item != null)
                <tr>
                  <td>{{ $loop->iteration }}</td>
                  <td>{{$item->book_category_name}}</td>
                  <td class=" text-center">
                    ...
                    <ul id='dropdown1{{$item->id}}' class='dropdown-content custom-dropdown-for-action'>
                      <li>
                        @if(Auth::user()->can('book-category-of-library-edit'))
                        <a href="{{ route('book-category-of-library.edit',$item->id) }}"><i class="material-icons">edit</i>Edit</a>
                        @endif
                      </li>
                      <li>
                        @if(Auth::user()->can('book-category-of-library-delete'))
                        <form id="delete-form" action="{{ route('book-category-of-library.destroy', $item->id) }}" method="POST"
