...
@section('content')
@section('title') Teacher Payment Invoice @endsection
                    ...
                    <div class="card-content invoice-print-area">
                            ...
                            <div class="col xl8 s12">
                                <div class="invoice-date display-flex align-items-center flex-wrap">
                                    ...
                                    <div class="media-print-date-section">
                                        ...
                                        <span>
                                            {{Carbon\Carbon::createFromFormat('Y-m-d', $invoiceData['date'])->format('d-m-Y')}}
                                        </span>
                                    </div>
                        ...
                        <div class="row invoice-info">
                            <div class="col m6 s12">
                                <h5 class="invoice-from custom-invoice-form">Bill From</h5>
                                <div class="invoice-address">
                                    <span>{{$invoiceData['name']}}</span>
                                </div>
                                <div class="invoice-address">
                                    @if(isset($invoiceData['address']) && $invoiceData['address'] != null)
                                ...
                                <div class="invoice-address">
                                    @if(isset($invoiceData['phone']) && $invoiceData['phone'] != null)
                            ...
                            <div class="col m6 s12 custom-text-right">
                                <h5 class="invoice-to custom-invoice-to">Bill To</h5>
                                <div class="invoice-address">
                                    <span>{{$invoiceData['teacher_name']}}</span>
                                </div>
                                <div class="invoice-address">
                                    @if(isset($invoiceData['teacher_address']) && $invoiceData['teacher_address'] != null)
                                    <span>{{$invoiceData['teacher_address']}}</span>
                                    @else
                                    <span>{{$invoiceData['teacher_address']}}</span>
                                    @endif
                                </div>
                                <div class="invoice-address">
                                    @if(isset($invoiceData['teacher_email']) && $invoiceData['teacher_email'] != null)
                                    <span>{{$invoiceData['teacher_email']}}</span>
                                    @else
                                    <span>sampleemail@gmail.com</span>
                                    @endif
                                </div>
                                <div class="invoice-address">
                                    @if(isset($invoiceData['teacher_phone']) && $invoiceData['teacher_phone'] != null)
                                    <span>{{$invoiceData['teacher_phone']}}</span>
                                    @else
                                    <span>+8801000000000</span>
                                    @endif
                                </div>
                        ...
                        <div class="invoice-product-details">
                            <table class="striped responsive-table custom-text-center custom-border-invoice-table">
                                <thead class="bg-light custom-bg-light">
                                    <tr>
                                        <th class="custom-text-center">#</th>
                                        <th class="custom-text-center">Payment Date</th>
                                        <th class="custom-text-center">Payment Method</th>
                                        <th class="custom-text-center" class="right-align">Payment Amount</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td class="custom-text-center">1</td>
                                        <td class="custom-text-center">{{Carbon\Carbon::createFromFormat('Y-m-d', $invoiceData['payment_date'])->format('d-m-Y')}}</td>
                                        <td class="custom-text-center">{{ $invoiceData['payment_method'] }}</td>
                                        <td class="custom-text-center" class="indigo-text right-align">{{ $invoiceData['payment_amount'] }}Tk</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
