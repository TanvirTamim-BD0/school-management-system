...
@section('session.index') active @endsection
@section('styles')
@endsection
@section('content')
    ...
    <div class="container">
      <div class="row">
        <div class="col s10 m6 l6">
          <h5 class="breadcrumbs-title mt-0 mb-0"><span>Session List</span></h5>
          <ol class="breadcrumbs mb-0">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a>
            </li>
            <li class="breadcrumb-item"><a href="#">Session</a>
            </li>
            <li class="breadcrumb-item active">Session List
            </li>
          </ol>
        </div>
      </div>
  ...
  <div class="col s12">
    <div class="card">
      ...
      <div class="card-content custom-card-content custom-card-content-for-datatable">
        <h2 class="card-title">Session Record List</h2>
        <div class="float-right justify-content-end">
          @if(Auth::user()->can('class-create'))
          <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button"
            href="{{route('session.create')}}">
            <i class="material-icons dp48">add_circle_outline</i> Add Session
          </a>
          @endif
        </div>
      </div>
        ...
        <table id="classTable"
          ...
          <thead>
            <tr>
              ...
              <th class="custom-border-right">Session</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class=" text-center">
                <!-- Dropdown Trigger -->
                <a class='dropdown-trigger btn custom-dropdown-btn' href='#' data-target='dropdown1{{$session->id}}'>
                  <i class="material-icons float-right">more_vert</i>
                </a>
                <!-- Dropdown Structure -->
                <ul id='dropdown1{{$session->id}}' class='dropdown-content custom-dropdown-for-action'>
                  <li>
                    @if(Auth::user()->can('class-edit'))
                    <a href="{{ route('session.edit',$session->id) }}"><i class="material-icons">edit</i>Edit</a>
                    @endif
                  </li>
                  <li>
                    @if(Auth::user()->can('class-delete'))
                    <form id="delete-form" action="{{ route('session.destroy', $session->id) }}" method="POST"
