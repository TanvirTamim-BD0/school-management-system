...
@section('syllabus.index') active @endsection
@section('styles')
@endsection
@section('content')
    ...
    <div class="container">
      <div class="row">
        <div class="col s10 m6 l6">
          <h5 class="breadcrumbs-title mt-0 mb-0"><span>Syllabus List</span></h5>
          <ol class="breadcrumbs mb-0">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a>
            </li>
            <li class="breadcrumb-item"><a href="#">Syllabus</a>
            </li>
            <li class="breadcrumb-item active">Syllabus List
            </li>
          </ol>
        </div>
      </div>
  ...
  <div class="col s12">
    <div class="card">
      <div class="card-content custom-card-content custom-card-content-for-datatable">
        <h2 class="card-title">Syllabus Record List</h2>
        <div class="float-right justify-content-end">
          @if(Auth::user()->can('syllabus-create'))
          <a class="waves-effect waves dark btn gradient-45deg-purple-deep-orange next-step add-attendance-button"
            href="{{route('syllabus.create')}}">
            <i class="material-icons dp48">add_circle_outline</i> Add Syllabus
          </a>
          @endif
        </div>
      </div>
        ...
        <div class="table-responsive custom-table-modify">
          <table id="myTable" class="table table-bordered custom-table-border">
            ...
            <tr>
              ...
              <th class="custom-border-right">Syllabus</th>
          ...
          <tbody>
            ...
            @foreach($syllabusData as $key => $item)
            @if(isset($item) && $item != null)
            <tr>
              <td>{{ $loop->iteration }}</td>
              <td>{{$item->classData->class_name}}</td>
              <td>
                Title: <span class="custom-text-primary">{{$item->title}}</span> <br>
                Description: <span class="custom-text-primary">{!! Str::limit($item->description, 250) !!}</span>,
              </td>

              <td>
                <a href="{{route('syllabus-file-download', $item->id)}}">
                  {{$item->syllabus_file}}
                </a>
              </td>
              ...
              <td class=" text-center">
                ...
                <ul id='dropdown1{{$item->id}}' class='dropdown-content custom-dropdown-for-action'>
                  <li>
                    ...
                    <a href="{{ route('syllabus.edit',$item->id) }}"><i class="material-icons">edit</i>Edit</a>
                    @endif
                  </li>
                  <li>
                    @if(Auth::user()->can('syllabus-delete'))
                    <form id="delete-form" action="{{ route('syllabus.destroy', $item->id) }}" method="POST"
